...
func (s *Server) VaultTokenAssign(h http.Handler) http.HandlerFunc {
	return http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			sendErr(l, w, 500, "code ends here for now")
		})
}
...
func (s *Server) JWTExtract(h http.Handler) http.HandlerFunc {
	...
	return http.HandlerFunc(
		func(w http.ResponseWriter, r *http.Request) {
			...
			var JWT *jwt.Token
			if *devMode {
				JWT = jwt.NewWithClaims(jwt.SigningMethodRS384, jwt.StandardClaims{
					Audience:  "all",
					ExpiresAt: time.Now().Add(10 * time.Hour).Unix(),
					Id:        "debug",
					IssuedAt:  time.Now().Unix(),
					Issuer:    "debug",
					NotBefore: time.Now().Add(-10 * time.Hour).Unix(),
					Subject:   "debug-account",
				})
			}
			...
			if authHeader := r.Header.Get("Authorization"); authHeader != "" {
				...
				tok, _, err := jwtParser.ParseUnverified(authHeader[7:], nil)
				if err != nil {
			...
			if JWT == nil {
				...
				if c, _ := r.Cookie("vault_jwt_proxy_auth"); c != nil && c.Value != "" {
					...
					tok, _, err := jwtParser.ParseUnverified(c.Value, nil)
					if err != nil {
						sendErr(l.With(zap.Error(err)), w, 400, "invalid jwt given")
						return
					}
			...
			_, r = enrichLoggerForReq(r, zap.String("jwt", JWT.Raw))
